...
func (u *User) CreateContactDetail(contactId int, paramType string, value interface{}) {
	if !u.CheckForisAdminAndIsActiveForUser() {
		return
	}
	if contactId < 0 || contactId >= len(u.Contacts) {
...
func (u *User) GetAllContactDetails() *[]contact_detail.ContactDetail {
	if !u.CheckForisAdminAndIsActiveForUser() {
		return nil
	}
	var contactDetails []contact_detail.ContactDetail
	for _, contact := range u.Contacts {
		if contact.IsActive {
